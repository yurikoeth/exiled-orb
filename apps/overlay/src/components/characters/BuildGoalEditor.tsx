    ...
    <Panel bg="flat" gold={!!goal} className="px-2 py-1.5">
      ...
      {expanded && (
        <div className="mt-2 space-y-2">
          <Field label="Build Name" value={buildName} onChange={setBuildName} placeholder="e.g. RF Juggernaut, Lightning Arrow Deadeye" />

          <div>
            <div className="text-xs mb-0.5" style={{ color: "var(--text-secondary)" }}>Focus</div>
            <div className="flex flex-wrap gap-1">
              {FOCUS_OPTIONS.map((f) => (
                <Btn key={f} variant="gold" active={focus.includes(f)} onClick={() => toggleFocus(f)}
                  style={focus.includes(f)
          ...
          <Field label="Budget" value={budget} onChange={setBudget} placeholder="e.g. 50 divine, 2000 chaos, unlimited" />
          <Field label="Notes" value={notes} onChange={setNotes} placeholder="e.g. need more chaos res, want to do ubers" />

          <Btn variant="gold" size="action" className="w-full" style={{ background: "rgba(255,255,255,0.06)" }} onClick={save}>
            Save Goal
          </Btn>
        </div>
      )}
